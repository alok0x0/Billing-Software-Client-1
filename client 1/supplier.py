...
class supplierClass:
    def __init__(self,root):
        ...
        btn_clear=Button(self.root, text="Show All",command=self.show,font=("goudy old style",15),bg="#607d8b",fg="white",cursor="hand2").place(x=180,y=420,width=110,height=35)
    ...
    def update(self):
        ...
        try:
            if self.var_sup_invoice.get()=="":
                ...
                row=cur.fetchone()
                if row==None:
                    messagebox.showerror("Error","Invalid Invoie no.",parent=self.root)
                else:
