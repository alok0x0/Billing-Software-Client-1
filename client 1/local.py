...
class localClass:
    def __init__(self,root):
        ...
        self.root.title("KST Inventory Management System")
        ...
        #=====Title======
        lbl_title = Label(self.root, text="Local Customer Bills", font=("goudy old style", 30), bg="#184a45",fg="white", bd=3, relief=RIDGE).pack(side=TOP, fill=X, padx=10, pady=20)
        ...
        self.Sales_List=Listbox(sales_Frame,font=("goudy old sytle",15),bg="white",yscrollcommand=scrolly.set)
        ...
        lbl_title2=Label(bill_Frame, text="View Customer Bill Area", font=("goudy old style",20), bg="orange").pack(side=TOP, fill=X)
                
        scrolly2= Scrollbar(bill_Frame, orient=VERTICAL)
        self.bill_area=Text(bill_Frame, font=("goudy old sytle", 10), bg="lightyellow", yscrollcommand=scrolly2.set)
...
#==================================================================================================
    ...
    def get_data(self,ev):
        ...
        fp=open(f'localbill/{file_name}','r')
        for i in fp:
    ...
    def search(self):
        if self.var_invoice.get()=="":
            ...
            if self.var_invoice.get() in self.bill_list:
                fp = open(f'localbill/{self.var_invoice.get()}.txt', 'r')
                self.bill_area.delete('1.0',END)
                for i in fp:
                    self.bill_area.insert(END, i)
                fp.close()
