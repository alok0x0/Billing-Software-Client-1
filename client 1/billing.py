...
class BillClass:
    def __init__(self, root):
        ...
        self.productTable = ttk.Treeview(ProductFrame3, columns=("pid", "name", "price","dis","sgst","cgst","igst", "qty", "status"),
        ...
        BTitle = Label(billFrame, text="Customer Bills Area", font=("goudy old style", 20, "bold"), bg="#f44336",
                       fg="white").pack(side=TOP, fill=X)
        ...
        btn_clear = Button(billMenuFrame, text='Clear All', cursor='hand2', command=self.clear_all,
                           ...
                           bg="gray", fg="white")
        btn_clear.place(x=124, y=5, width=120, height=70)
        ...
        btn_print = Button(billMenuFrame, text='Local Bill',command=self.bill_local,cursor='hand2', font=("gaudy old style", 15, 'bold'),
                              bg="lightgreen", fg="white")
    ...
    def show(self):
        ...
        cur = con.cursor()
        try:
            cur.execute("select pid, name, price,dis,sgst,cgst,igst, qty, status from product where status = 'Active'")
            rows = cur.fetchall()
    ...
    def search(self):
        ...
        try:
            if self.var_search.get() == "":
                ...
                cur.execute("select pid, name, price,dis,sgst,cgst,igst, qty, status from product where name LIKE '%" + self.var_search.get() + "%' and status = 'Active'")
                rows = cur.fetchall()
                if len(rows) != 0:
                    self.productTable.delete(*self.productTable.get_children())
                    for row in rows:
    ...
    def add_update_cart(self):
        if self.var_pid.get()=='':
            ...
            cart_data = [self.var_pid.get(), self.var_pname.get(), price_cal,self.var_dis.get(),self.var_sgst.get(),self.var_cgst.get(),self.var_igst.get(), self.var_qty.get(),self.var_stock.get()]
            #==========Update_cart===============
            present = 'no'
            index_ = 0
...
# ==================bill Bottom===============
            ...
            fp=open(f'localbill/{str(self.invoice)}.txt','w')
...
# ==================bill Bottom===============
            ...
            cur.execute("insert into generatebill4(InvoiceNo,CustomerName,Date,Qty,Amt,dis,sgst,cgst,igst,Netamount) values(?,?,?,?,?,?,?,?,?,?)",
                (
                 self.invoice,
                 self.var_cname.get(),
                 self.date_,
