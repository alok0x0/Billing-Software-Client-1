...
class productClass:
    def __init__(self, root):
        ...
        product_Frame.title("KST Inventory Management System")
        ...
        self.productTable = ttk.Treeview(p_frame,
                                         columns=("pid", "Supplier", "Category", "name", "price","DIS","SGST","CGST","IGST", "qty", "status"),
        ...
        try:
            if self.var_cat.get() == "Select" or self.var_cat.get() == "Empty" or self.var_sup.get() == "Select" or self.var_name.get() == "":
                ...
                if row != None:
                    ...
                    cur.execute("insert into product(Category, Supplier, name, price,dis,sgst,cgst,igst, qty, status) values(?,?,?,?,?,?,?,?,?,?)",
                        (
                            self.var_cat.get(),
                            self.var_sup.get(),
                            self.var_name.get(),
        ...
        try:
            if self.var_pid.get() == "":
                ...
                if row == None:
                    ...
                    cur.execute(
                        "Update product set Category=?,Supplier=?,name=?,price=?,dis=?,sgst=?,cgst=?,igst=?,qty=?,status=? where pid=?",
                        (
                            self.var_cat.get(),
                            self.var_sup.get(),
                            self.var_name.get(),
